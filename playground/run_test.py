...
def runIperfTest(net, hosts_src, hosts_dst, switches_list):
    ...
    # check that test mode is either mix or tcp
    mode = test_mode
    if mode not in ["mix", "tcp"]:
        print("  Test mode specified should be either `mix` or `tcp`")
        print("  Going ahead with default `tcp`")
        mode = "tcp"
    ...
    for i in range(0, num_hostpairs): # for each TCP host pair
        h = hosts_dst[i]
        cmd_str = ("iperf -s -p 5003 -i 1 " +
              ("-u " if (mode == "mix" and i > 0) else "") +
              " > " + tmp_folder + h.name +
              tcp_server_throughput_suffix + " &")
        if debug_mode:
            print("Sending command", cmd_str, "to destination", i)
        h.cmd(cmd_str)

    print("  Running tcp_probe for TCP parameter captures")
    traced_events = subprocess.check_output("sudo cat /sys/kernel/debug/tracing/set_event",
                                            shell=True)
    tcp_probe_enabled = "tcp:tcp_probe" in str(traced_events)
    if not tcp_probe_enabled:
        print("  Traced events should include tcp:tcp_probe, tcp diagnostics unavailable.")
    ...
    for i in range(0, num_hostpairs):
        ...
        cmd_str = ("iperf -t " + str(test_duration_sec) + " -c " +
                   hdst.IP() + " -p 5003 -i 1 " +
                   ("-u" if (mode == "mix" and i > 0) else ("-Z " + tcp_flavor)) +
                   " > " +
...
def sharingTest():
    ...
    if debug_mode:
        for i in range(0, len(hosts_src)):
            print("Added host pair %s %s" % (
                str(hosts_src[i].IP()), str(hosts_dst[i].IP())))

    print("Starting traffic workload (iperf)")
    runIperfTest(net, hosts_src, hosts_dst, switches_list)
    
    print("Collecting statistics... this will run for", end=' ')
