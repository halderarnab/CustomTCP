...
# Re-capture tcp_probe outputs, just in case
sudo cat /sys/kernel/debug/tracing/trace > ${tmp_folder}${tcp_probe_file}

python3 throughput-plot.py
python3 drop-plot.py

# Drop rate
python3 $plotscript -a "font \"Times,24\"" \
       -p 1 -f "png" \
